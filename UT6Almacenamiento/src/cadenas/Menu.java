...
public class Menu {
	...
	public void añadirOpcion(String opcion) throws MaxOpcionesSuperado {
		...
		if (this.numOpciones < MAX_OPCIONES) {
			...
			throw new MaxOpcionesSuperado("Máximo de opciones ("
					+ this.MAX_OPCIONES + ") superado.");
	...
	public void eliminarOpcion(int posicion) {
		...
		if (this.numOpciones == 0) {
			...
			if (posicion >= this.numOpciones) {
				System.out
						.println("No hay opciones que eliminar en la posición elegida.");
				System.exit(0);
			}
			...
			try {
				...
				if (elegida <= 0 || elegida > this.opciones.length) {
					...
					if (this.opciones[elegida - 1] == null
							|| elegida > this.opciones.length) {
						System.out.println("La opción no existe.");
						valido = false;
					} else
						System.out.println("Ha elegido la opción \""
								+ this.opciones[elegida - 1] + "\"");
				}
